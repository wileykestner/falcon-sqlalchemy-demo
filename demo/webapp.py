...
def get_app():
    my_session_provider = SessionProvider()
    person_list_resource = PersonListResource(session_provider=my_session_provider)
    person_resource = PersonResource(session_provider=my_session_provider)

    app = falcon.API()
    app.add_route("/people", person_list_resource)
    app.add_route("/people/{identifier}", person_resource)
